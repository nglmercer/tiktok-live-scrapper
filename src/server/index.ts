...
app.get('/', (c) => {
  return c.text('TikTok LIVE WebSocket Server running - Connect via WebSocket to /ws');
});
...
app.post('/translate', async (c) => {
  ...
  if (contentType.includes('application/json')) {
    ...
    try {
      ...
      const result = await ProtobufService.decode({ buffer: Array.from(binaryData) });
      ...
      return c.json({
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error processing binary'
      }, 500);
    }
  } else {
    return c.json({
      success: false,
      error: 'Unsupported content type. Use application/json with base64/buffer or application/octet-stream'
    }, 400);
  }
});
      ...
      onMessage(event, ws) {
        try {
          ...
          if (typeof event.data === 'string') {
            try {
              ...
              if (jsonData.action === 'decode') {
                if (jsonData.base64) {
                  ...
                  ws.send(JSON.stringify({ error: 'Missing "base64" or "buffer" to decode' }));
                  return;
                }

                ProtobufService.decode({ base64: jsonData.base64 })
                  .then(result => {
                    if (result.success) {
                ...
                if (!jsonData.protoName || !jsonData.data) {
                  ws.send(JSON.stringify({ error: 'Missing "protoName" and "data" to encode' }));
                  return;
                }

                ProtobufService.encode(jsonData.protoName, jsonData.data)
                  .then(result => {
                    if (result.success) {
                      ws.send(JSON.stringify({
                        action: 'encoded',
              ...
              ProtobufService.decode({ base64: event.data })
                .then(result => {
                  if (result.success) {
                    ws.send(JSON.stringify({
                      action: 'decoded',
            ...
            ProtobufService.decode({ buffer: Array.from(binaryData) })
              .then(result => {
                if (result.success) {
                  ws.send(JSON.stringify({
                    action: 'decoded',
...
console.log(`  POST http://localhost:${PORT}/translate - Decode protobuf from buffer or base64`);
console.log(`  POST http://localhost:${PORT}/encode - Encode JSON to protobuf`);
console.log(`  WS ws://localhost:${PORT}/ws - Real-time translation via WebSocket`);
...
if (hasBun) {
  globalThis.Bun.serve({
    ...
    console.log(`HTTP server listening on http://localhost:${serverInstance.port}`);
    console.log(`WebSocket ready at ws://localhost:${serverInstance.port}/ws`);
  });
} else {
  serve({
    port: PORT,
